...
static double Physac_GetCurrentTime(void);         // Get current time measure in milliseconds
#endif
...
void UpdatePhysics(void)
{
    ...
    currentTime = Physac_GetCurrentTime();
...
static void InitTimerHiRes(void)
{
    ...
    startTime = Physac_GetCurrentTime();      // Get current time in milliseconds
}
...
static double Physac_GetCurrentTime(void)
{
...
static void SolveCircleToPolygon(PhysicsManifold manifold)
{
    ...
    else // Closest to face
    {
        ...
        if (MathVector2DotProduct(MathVector2Subtract(center, v1), normal) > bodyA->shape.radius)
            return;
...
static void SolvePolygonToPolygon(PhysicsManifold manifold)
{
    ...
    if (MathVector2Clip(CLITERAL(Vector2){-sidePlaneNormal.x, -sidePlaneNormal.y}, &incidentFace[0], &incidentFace[1], negSide) < 2)
        return;
...
static void IntegratePhysicsForces(PhysicsBody body)
{
    ...
    if (!body->freezeOrient)
        body->angularVelocity += (float)(body->torque * body->inverseInertia * (deltaTime / 2.0));
}
...
static void InitializePhysicsManifolds(PhysicsManifold manifold)
{
    ...
    for (unsigned int i = 0; i < manifold->contactsCount; i++)
    {
        ...
        if (MathVector2SqrLen(radiusV) < (MathVector2SqrLen(CLITERAL(Vector2){(float)(gravityForce.x * deltaTime / 1000), (float)(gravityForce.y * deltaTime / 1000)}) + PHYSAC_EPSILON))
    ...
    for (unsigned int i = 0; i < manifold->contactsCount; i++)
    {
        ...
        if (bodyA->enabled)
        {
            ...
            if (!bodyA->freezeOrient)
                bodyA->angularVelocity += bodyA->inverseInertia * MathVector2CrossProduct(radiusA, CLITERAL(Vector2){-impulseV.x, -impulseV.y});
        }
        ...
        if (absImpulseTangent < impulse * manifold->staticFriction)
            ...
            tangentImpulse = CLITERAL(Vector2){tangent.x * -impulse * manifold->dynamicFriction, tangent.y * -impulse * manifold->dynamicFriction};
        ...
        if (bodyA->enabled)
        {
            ...
            if (!bodyA->freezeOrient)
                bodyA->angularVelocity += bodyA->inverseInertia * MathVector2CrossProduct(radiusA, CLITERAL(Vector2){-tangentImpulse.x, -tangentImpulse.y});
        }
...
static void IntegratePhysicsVelocity(PhysicsBody body)
{
    ...
    if (!body->freezeOrient)
        body->orient += (float)(body->angularVelocity * deltaTime);
