...
RMAPI Matrix MatrixFrustum(double r_left, double r_right, double r_bottom, double r_top, double r_near, double r_far)
{
    Matrix result = {0};

    float rl = (float)(r_right - r_left);
    float tb = (float)(r_top - r_bottom);
    float fn = (float)(r_far - r_near);

    result.m0 = ((float)r_near * 2.0f) / rl;
    result.m1 = 0.0f;
    result.m2 = 0.0f;
    result.m3 = 0.0f;

    result.m4 = 0.0f;
    result.m5 = ((float)r_near * 2.0f) / tb;
    result.m6 = 0.0f;
    result.m7 = 0.0f;

    result.m8 = ((float)r_right + (float)r_left) / rl;
    result.m9 = ((float)r_top + (float)r_bottom) / tb;
    result.m10 = -((float)r_far + (float)r_near) / fn;
    result.m11 = -1.0f;

    result.m12 = 0.0f;
    result.m13 = 0.0f;
    result.m14 = -((float)r_far * (float)r_near * 2.0f) / fn;
    result.m15 = 0.0f;
